...
addons-downloaded-fnames = Letöltve %(fname)s
addons-downloading-adbd-kb02fkb = Letöltve %(a)d/%(b)d (%(kb)0.2fKB)...
addons-error-downloading-ids-errors = Hiba történt a letöltéskor <i>%(id)s</i>: %(error)s
addons-error-installing-bases-errors = Hiba történt a telepítéskor <i>%(base)s</i>: %(error)s
...
addons-installed-names = Telepítve %(name)s
...
addons-the-following-addons-are-incompatible-with = A következő kiegészítők nem kompatibilisek a %(name)s névvel és le vannak tiltva: %(found)s
