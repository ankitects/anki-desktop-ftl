addons-possibly-involved = 可能涉及的附加元件: { $addons }
addons-failed-to-load =
    您安裝的一個附加元件載入失敗了。如果問題持續出現，請前往「工具」>「附加元件」選單，並停用或刪除附加元件。
    
    載入「{ $name }」時:
    { $traceback }
# Shown in the add-on configuration screen (Tools>Add-ons>Config), in the title bar
addons-config-window-title = 設定「{ $name }」
addons-config-validation-error = 提供的設定存在問題: { $problem }，位於路徑{ $path }，對於結構描述{ $schema }。
addons-window-title = 附加元件
addons-addon-has-no-configuration = 附加元件沒有設定檔。
addons-addon-installation-error = 附加元件安裝出錯
addons-addon-was-not-downloaded-from-ankiweb = 附加元件不是從 AnkiWeb 下載的。
addons-browse-addons = 瀏覽附加元件
addons-changes-will-take-effect-when-anki = 變更將在 Anki 重新啟動後生效。
addons-check-for-updates = 檢查更新
addons-checking = 檢查中...
addons-code = 代碼:
addons-config = 設定
addons-configuration = 設定
addons-corrupt-addon-file = 損毀的附加元件檔案。
addons-disabled = (已停用)
addons-disabled2 = (已停用)
addons-download-complete-please-restart-anki-to = 下載完成。請重新啟動 Anki 以套用變更。
addons-downloaded-fnames = 已下載 %(fname)s
addons-downloading-adbd-kb02fkb = 下載中 %(a)d/%(b)d (%(kb)0.2fKB)...
addons-error-downloading-ids-errors = 下載 <i>%(id)s</i> 時發生錯誤: %(error)s
addons-error-installing-bases-errors = 安裝 <i>%(base)s</i> 時發生錯誤: %(error)s
addons-get-addons = 取得附加元件...
addons-important-as-addons-are-programs-downloaded = <b>非常重要</b>: 附加元件是來自網際網路的程式，因此有可能是惡意程式。<b>您應該只安裝信任的附加元件。</b><br><br>您確定要繼續安裝以下 Anki 附加元件嗎？<br><br>%(names)s
addons-install-addon = 安裝附加元件
addons-install-addons = 安裝附加元件
addons-install-anki-addon = 安裝 Anki 附加元件
addons-install-from-file = 從檔案安裝...
addons-installation-complete = 安裝完成
addons-installed-names = 已安裝 %(name)s
addons-installed-successfully = 安裝成功。
addons-invalid-addon-manifest = 無效的附加元件資訊清單。
addons-invalid-code = 無效代碼。
addons-invalid-code-or-addon-not-available = 無效代碼，或附加元件不可用於此版本的 Anki。
addons-invalid-configuration = 無效的設定:
addons-invalid-configuration-top-level-object-must = 無效設定: 頂層物件必須為 map
addons-no-updates-available = 無可用更新。
addons-one-or-more-errors-occurred = 發生了一個或多個錯誤:
addons-packaged-anki-addon = 已封裝的 Anki 附加元件
addons-please-check-your-internet-connection = 請檢查您的網際網路連線。
addons-please-report-this-to-the-respective = 請將此內容回報到對應的附加元件作者。
addons-please-restart-anki-to-complete-the = <b>請重新啟動 Anki 來完成安裝。</b>
addons-please-select-a-single-addon-first = 請先選取單個附加元件。
addons-requires = (需要{ $val })
addons-restored-defaults = 已恢復預設
addons-the-following-addons-are-incompatible-with = 以下附加元件與 %(name)s 不相容，且已停用: %(found)s
addons-the-following-addons-have-updates-available = 以下附加元件有可用的更新。是否立即安裝？
addons-the-following-conflicting-addons-were-disabled = 已停用以下衝突的附加元件:
addons-this-addon-is-not-compatible-with = 此附加元件與您的 Anki 版本不相容。
addons-to-browse-addons-please-click-the = 要瀏覽附加元件，請按下方的瀏覽按鈕。<br><br>當您找到喜歡的附加元件時，請將它的代碼在下方貼上。您可以貼上多條代碼，以空格分開。
addons-toggle-enabled = 切換啟用狀態
addons-unable-to-update-or-delete-addon = 無法更新或刪除附加元件。請在開啟 Anki 時按住 Shift 鍵以暫時停用附加元件，然後再試一次。  除錯資訊: { $val }
addons-unknown-error = 未知錯誤: { $val }
addons-view-addon-page = 檢視附加元件頁面
addons-view-files = 檢視檔案
addons-delete-the-numd-selected-addon =
    { $count ->
       *[other] 刪除已選取的 { $count } 個附加元件？
    }
