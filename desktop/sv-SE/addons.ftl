...
addons-addon-was-not-downloaded-from-ankiweb = Tillägget laddades inte ned från AnkiWeb.
...
addons-downloaded-fnames = Laddade ned %(fname)s
addons-error-installing-bases-errors = Fel vid installering av <i>%(base)s</i>%(error)s
...
addons-installed-names = Installerat %(name)s
...
addons-the-following-addons-are-incompatible-with = De följande tilläggen är inkompatiibla med %(name)s och har inaktiverats: %(found)s
