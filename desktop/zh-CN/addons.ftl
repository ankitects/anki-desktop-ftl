...
addons-browse-addons = 浏览插件
addons-changes-will-take-effect-when-anki = 重启 Anki 以应用更改
addons-check-for-updates = 检查更新
addons-checking = 正在检查...
addons-code = 代码:
addons-config = 设置
addons-configuration = 设置
addons-corrupt-addon-file = 无效的扩展文件。
addons-disabled = （禁用）
...
addons-packaged-anki-addon = 已打包的Anki扩展组件
addons-please-check-your-internet-connection = 请检查您的互联网连接。
addons-please-report-this-to-the-respective = 请将此报告给相应的扩展组件开发者
...
addons-to-browse-addons-please-click-the = 浏览插件请点击下方的“浏览”按钮。<br><br>如果您找到适合您的插件，请将插件代码粘贴到下方的文本框中。您可以输入多个代码，请将代码之间用空格隔开。
addons-toggle-enabled = 切换启用状态
addons-unable-to-update-or-delete-addon = 无法更新或删除扩展组件。请在按下Shift键的同时打开Anki以临时禁用所有扩展组件，然后重试一次。 调试信息: { $val }
...
addons-choose-update-window-title = 更新附加组件
