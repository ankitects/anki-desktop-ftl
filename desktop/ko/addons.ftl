...
addons-addon-was-not-downloaded-from-ankiweb = 확장 프로그램이 AnkiWeb에서 다운로드 되지 않았습니다.
...
addons-downloaded-fnames = %(fname)s 다운로드됨
addons-downloading-adbd-kb02fkb = 다운로드 중 %(a)d/%(b)d (%(kb)0.2fKB)...
addons-error-downloading-ids-errors = 다운로드 오류 <i>%(id)s</i>: %(error)s
addons-error-installing-bases-errors = 설치 오류 <i>%(base)s</i>: %(error)s
...
addons-installed-names = %(name)s 이/가 설치되었습니다.
...
addons-the-following-addons-are-incompatible-with = 해당 확장 프로그램은 %(name)s 와 호환되지 않아 비활성화 되었습니다: %(found)s
