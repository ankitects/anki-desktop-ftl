qt-misc-choose-deck = ତାସଖଣ୍ଡ ବାଛନ୍ତୁ
qt-misc-choose-note-type = ନୋଟ୍ ପ୍ରକାର ବାଛନ୍ତୁ
qt-misc-closing = ବନ୍ଦ କରୁଛି...
qt-misc-copy-to-clipboard = କ୍ଲିପବୋର୍ଡରେ କପି କରିନିଅନ୍ତୁ
qt-misc-manage = ପରିଚାଳନା
qt-misc-manage-note-types = ନୋଟ୍ ପ୍ରକାରଗୁଡ଼ିକର ପରିଚାଳନା
qt-misc-name-exists = ନାମ ବିଦ୍ୟମାନ ଅଛି।
qt-misc-please-select-a-deck = ଦୟାକରି ଗୋଟେ ତାସଖଣ୍ଡ ବାଛନ୍ତୁ।
qt-misc-processing = ପ୍ରକ୍ରିୟାରତ...
qt-misc-replace-your-collection-with-an-earlier = ଆପଣଙ୍କ ସଂଗ୍ରହକୁ ଏକ ପୂର୍ବବ୍ୟାକଅପ୍ ସହ ବଦଳାଇବେ କି?
qt-misc-stats = ପରିସଂଖ୍ୟାନ
qt-misc-there-must-be-at-least-one = ଅତି କମରେ ଗୋଟିଏ ପ୍ରୋଫାଇଲ୍ ରହିବା ଆବଶ୍ୟକ।
qt-misc-your-firewall-or-antivirus-program-is = ଆପଣଙ୍କ ଫାୟାରୱାଲ୍ କିମ୍ୱା ଆଣ୍ଟିଭାଇରସ୍ ପ୍ରୋଗ୍ରାମ୍ Ankiକୁ ନିଜ ସହିତ ଏକ ସଂଯୋଗ ସୃଷ୍ଟି କରିବାକୁ ଦେଉନି। ଦୟାକରି Anki ପାଇଁ ଏକ ବ୍ୟତିକ୍ରମ ଯୋଡ଼ନ୍ତୁ।
qt-misc-error = ତ୍ରୁଟି
qt-misc-anki-is-running = Anki ଆଗରୁ ଚାଲୁଛି
qt-misc-second =
    { $count ->
        [one] { $count } ସେକେଣ୍ଡ
       *[other] { $count } ସେକେଣ୍ଡ
    }

## deprecated- these strings will be removed in the future, and do not need
## to be translated

