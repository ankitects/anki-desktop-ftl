about-a-big-thanks-to-all-the = Ұсыныс білдіріп, қателер туралы хабарлап, қолдау көрсеткен барлық жанға зор алғыс!
about-about-anki = Anki Жайында
about-anki-is-a-friendly-intelligent-spaced = Anki — ыңғайлы әрі зерделі аралық қайталау жүйесі. Ол ашық және тегін бағдарлама.
about-anki-is-licensed-under-the-agpl3 = Anki AGPL3 лицензиясымен таратылады. Толық мәліметті бастапқы кодтағы лицензия файлынан қараңыз.
about-copied-to-clipboard = Буферге көшірілді
about-copy-debug-info = Оңдау Дерегін Көшіру
about-if-you-have-contributed-and-are = Егер сіз үлес қосқан болсаңыз, бірақ бұл тізімде жоқ болсаңыз — хабарласыңыз.
about-version = Нұсқа { $val }
about-visit-website = <a href='{ $val }'>Сайтқа өту</a>
about-written-by-damien-elmes-with-patches = Дәмиен Элмс жасаған, ал түзетулер, аударма,     сынақ және дизайн авторлары:<p>{ $cont }
# appended to the end of the contributor list in the about screen
about-and-others = және басқалар
